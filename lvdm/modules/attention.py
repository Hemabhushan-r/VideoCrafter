...
CROSS_ATTN_TEXT_CONTEXT_LEN = int(os.environ['CROSS_ATTN_TEXT_CONTEXT_LEN'])

print(f'CROSS_ATTN_TEXT_CONTEXT_LEN: {CROSS_ATTN_TEXT_CONTEXT_LEN}')
...
class CrossAttention(nn.Module):
    ...
    def __init__(self, query_dim, context_dim=None, heads=8, dim_head=64, dropout=0.,
        ...
        self.text_context_len = CROSS_ATTN_TEXT_CONTEXT_LEN
    ...
    def forward(self, x, context=None, mask=None):
        ...
        if context is not None and self.img_cross_attention:
            context, context_img = context[:, :self.text_context_len,
        ...
        if self.relative_position:
            ...
            sim2 = einsum('b t d, t s d -> b t s', q, k2) * \
                self.scale  # TODO check
            sim += sim2
        del k
        ...
        # considering image token additionally
        if context is not None and self.img_cross_attention:
            k_ip, v_ip = map(lambda t: rearrange(
                t, 'b n (h d) -> (b h) n d', h=h), (k_ip, v_ip))
            sim_ip = torch.einsum('b i d, b j d -> b i j',
                                  q, k_ip) * self.scale
            del k_ip
...
class BasicTransformerBlock(nn.Module):
    ...
    def _forward(self, x, context=None, mask=None):
        x = self.attn1(self.norm1(
            x), context=context if self.disable_self_attn else None, mask=mask) + x
...
class TemporalTransformer(nn.Module):
    ...
    def forward(self, x, context=None):
        ...
        if self.only_self_att:
            ...
            context = repeat(
                context, 'b t l con -> b (t r) l con', r=(h * w) // t, t=t).contiguous()
            context = rearrange(
                context, 'b t l con -> (b t) l con').contiguous()
            for i, block in enumerate(self.transformer_blocks):
